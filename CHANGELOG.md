...
✨ feat: implement topology enforcer, governance proxy, AMM pool, and Me…
• Merge branch 'main' into feat/topology-enforcer-governance-proxy-amm-merkle-proof
• Merge pull request #140 from Priscy-bells/docs/operational-excellence
📝 docs: comprehensive operational excellence documentation and examples
✨ feat: implement topology enforcer, governance proxy, AMM pool, and Merkle proof primitives Issue #115 - [Backend] Dynamic Node Topology Spread Constraint Enforcer - src/controller/topology/rules.rs: ClusterTopology detection, TopologySpreadConstraint builders, hard/soft anti-affinity helpers, build_rule_set() with HardMultiZone (3+ zones) and SoftSingleZone (<3 zones / dev clusters) modes, full unit-test suite - src/controller/topology/enforcer.rs: discover_cluster_topology() inspects live node zone labels, build_statefulset_patch() generates JSON patch with topologySpreadConstraints + podAntiAffinity, enforce_on_statefulset() and enforce_namespace() apply patches via server-side apply, dry-run support, EnforcementResult reporting - src/controller/topology/mod.rs: module wiring and re-exports Issue #110 - [Contract] Upgradable Governance Proxy with Storage Layout Protection - src/contracts/upgrade_proxy/storage.rs: ContractStorage with impl:/gov: key-prefix namespace isolation, WasmRecord and PendingUpgrade domain types, typed accessors, collision-proof key helpers, full unit-test suite - src/contracts/upgrade_proxy/lib.rs: UpgradeProxy with initialize(), propose_upgrade(), execute_upgrade() (snapshot prev + activate new + increment counter + clear proposal), rollback_upgrade() (restore prev + decrement counter), add_governance_key(), auth guards, state-persistence-across-upgrade and layout-collision-impossible tests Issue #114 - [Contract] Dynamic Liquidity Pool AMM Primitive - src/contracts/amm_pool/math.rs: iterative isqrt (Babylonian), bps_to_scale(), fee helpers, get_amount_out() and get_amount_in() with constant-product formula, initial_lp_shares() (sqrt(x*y) - MINIMUM_LIQUIDITY), subsequent_lp_shares(), lp_to_amounts(), verify_k_invariant(), full arithmetic unit-test suite - src/contracts/amm_pool/lib.rs: AmmPool state machine — initialise(), add_liquidity(), remove_liquidity(), swap_x_for_y(), swap_y_for_x(), quote helpers; MINIMUM_LIQUIDITY lock, slippage protection via min_output, k-invariant verified after every swap, PoolEvent log Issue #113 - [Contract] Off-Chain State Merkle Proof Verification Primitive - src/contracts/merkle_proof/verify.rs: SHA-256 and Keccak-256 hash dispatch, domain-separated leaf/node hashing (0x00/0x01 prefixes), iterative compute_root() with odd-level duplication, verify_proof() single-leaf iterative path verification, verify_multiproof(), generate_proof() test helper, correctness tests across tree depths 2-32 - src/contracts/merkle_proof/lib.rs: MerkleVerifier with epoch-keyed root registry, TTL expiration, submit_root()/overwrite_root()/evict_expired(), verify() and verify_multi() with authorization and expiry checks Supporting changes: - Cargo.toml: add tiny-keccak 2.0 (keccak feature), fix pre-existing profile.release.package panic=abort manifest error (moved to [profile.release]) - src/lib.rs: expose contracts module - src/controller/mod.rs: expose topology module with re-exports Closes #115, #110, #114, #113
📝 docs: comprehensive operational excellence documentation and examples
• - docs/operations/upgrade-guide.md: Major version upgrade guide with step-by-step procedures including blue-green, canary, and rolling update strategies. Details database schema upgrades, rollback procedures, and emergency recovery.
• - docs/observability/telemetry-architecture.md: End-to-end observability reference with Prometheus, Loki, and Grafana configs. Includes exhaustive metric dictionaries for Stellar Core, Horizon, and Soroban RPC with alert thresholds.
• - docs/performance/rpc-optimization.md: Infrastructure optimization manual with hardware sizing for 100-10,000+ RPS tiers, Linux kernel tuning parameters, and Kubernetes configurations backed by empirical benchmarks.
• - examples/canary-deployment.yaml: Production-ready canary deployment manifests with Argo Rollouts, automated analysis templates, and rollback policies for zero-downtime upgrades.
• - examples/monitoring/prometheus-rules.yaml: Production-ready PrometheusRule manifests with validated alert thresholds for Stellar Core, Horizon, Soroban RPC, and infrastructure components.
• All documentation validated against real-world operational requirements and load test data.
