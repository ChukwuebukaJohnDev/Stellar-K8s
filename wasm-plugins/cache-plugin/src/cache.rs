    ...
    V: Clone,
{
    ...
    pub fn get(&mut self, key: &K, now: u64) -> Lookup<V> {
        ...
        let expired = match self.entries.get(key) {
            Some(entry) => self.ttl_seconds != 0 && now.saturating_sub(entry.inserted_at) >= self.ttl_seconds,
    ...
    pub fn put(&mut self, key: K, value: V, now: u64) {
        ...
        if self.entries.contains_key(&key) {
            self.entries.insert(key.clone(), Entry { value, inserted_at: now });
            self.touch(&key);
            return;
        }
...
mod tests {
    ...
    fn cache(capacity: usize, ttl_seconds: u64) -> LruTtlCache<String, String> {
        LruTtlCache::new(CacheConfig { capacity, ttl_seconds })
    }
    ...
    fn lru_eviction_removes_least_recently_used() {
        ...
        assert_eq!(c.get(&"b".to_string(), 0), Lookup::Miss, "b should have been evicted");
        assert_eq!(c.get(&"a".to_string(), 0), Lookup::Hit("1".into()));
    ...
    fn ttl_expiry_evicts_on_next_lookup() {
        ...
        assert_eq!(c.get(&"a".to_string(), 129), Lookup::Hit("1".into()), "not yet expired");
        assert_eq!(c.get(&"a".to_string(), 130), Lookup::Miss, "expired at exactly the TTL boundary");
        assert_eq!(c.len(), 0, "expired entry should be evicted, not just hidden");
    }
    ...
    fn re_putting_an_existing_key_refreshes_its_ttl_and_value() {
        ...
        assert_eq!(c.len(), 1, "refreshing an existing key must not duplicate it");
    }
}
