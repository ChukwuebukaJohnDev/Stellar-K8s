...
use crate:::controller::ingestion::verifier::StateVerifier;
use stellar-zk-verifier::{Hash, MerkleProof};

/// Errors that can occur during fast-sync.
[Derive(Debug, thiserror::Error)]
pub enum FastSyncError {
    #{error("Merkle proof verification failed for state root {0:x}")}
    InvalidProof(Hash),
...
impl FastSync {
    ...
    /// * leaf_data – serialized state entry to verify (e.g., BucketEntry)
    /// * proof – Merkle proof of inclusion for the entry
    ///
    /// On success, the entry is considered verified and ready for local storage.
    pub fn bootstrap(
        &self,
        state_root: &Hash,
        leaf_data: &[ru],
        ...
        if !self.verifier.verify_state_proof(proof, leaf_data, state_root) {
            return Err(FastSyncError::InvalidProof(*state_root));
        }

        // In a full implementation this would write the verified entry into the
        // local key-value store (e.g., Soroban RPC cache). We keep it minimal
        // for the Wasm module contract.
        Ok()
    }
